...
class TandaTerimaHistoryController extends Controller
{
    ...
    public function index()
{
    ...
    return view('tanda_terima_history.index', compact('vehicleTypes', 'memberTypes', 'companyList'));
}
    ...
    {
    $entryDate = $request->input('entry_date'); // format: YYYY-MM
    if ($entryDate) 
        {
            [$year, $month] = explode('-', $entryDate);
        }
    ...
    $vehicles = DB::table('rfid_vehicles')
        ...
        ->when($entryDate, function ($query) use ($year, $month) {
            $query->whereYear('member_history.created_at', $year)
                ->whereMonth('member_history.created_at', $month);})
        ->select(
                'vehicle_no',
                'company_name',
                'vehicleid',
    ...
    return view('tanda_terima_history.tanda_terima_history', compact('invoices'));
}
