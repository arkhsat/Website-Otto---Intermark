...
class ReportQtyPosExport3 implements FromView, WithDrawings, WithTitle, WithStyles
{
    protected $startDate;
    protected $endDate;
    protected $judul;
    ...
    public function __construct($pmm2_datahours, $startDate, $endDate, $judul)
    {   
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('A2:AA' . (count($this->pmm2) + 3))
              ->getBorders()
              ->getAllBorders()
              ->setBorderStyle(Border::BORDER_THIN);
        $sheet->getStyle('A1:AA' . (count($this->pmm2) + 3))
              ->getAlignment()
        ...
        $sheet->getStyle('A3:AA' . (count($this->pmm2) + 3))->getFont()->setSize(9);
        ...
        foreach ($this->pmm2 as $row => $result) {
            $excelRow = $row + 3;
