...
use App\Exports\ReportQtyMemberExport;
...
class ReportSummaryMemberController extends Controller
{
    public function getAmountReportMember(Request $request)
    {
    ...
    public function getPaymentReportMember(Request $request)
    {
        // if (\Auth::user()->can('manage report') ) {
        //     $report = Transaction::where('parent_id', '=', parentId())->get();
    ...
    public function getQtyReportMember(Request $request)
    {
        // if (\Auth::user()->can('manage report') ) {
        //     $report = Transaction::where('parent_id', '=', parentId())->get();
        ...
        return view('reportsummarymember.reportqtymember',compact('results'));
    }

    public function downloadpdfQtyMember(Request $request) {
        $startDate = $request->input('entry_date', date('Y-m-d'));
        $endDate = $request->input('end_date', date('Y-m-d'));
        ...
        $datahours = DB::select("
            ...
            SUM(CASE WHEN paymentby IN ('RFID','blue bird') THEN 1 ELSE 0 END) as total
            FROM transactions
        ...
        if ($startDate == $endDate) {
            $fileName = 'Intermark - Report QTY Member ' . date('d F Y', strtotime($startDate)) . '.pdf';
        } else {
            $fileName = 'Intermark - Report QTY Member' . date('d F Y', strtotime($startDate)) . ' sd ' . date('d F Y', strtotime($endDate)) . '.pdf';
        }
        $judul = 'Laporan QTY Transaksi Harian';

        $pdf = Pdf::loadView('PDF/pdfqtymember', ['data' => $results, 'datahours' => $datahours], compact('startDate', 'endDate', 'judul'))->setPaper('a4', 'landscape');


        return $pdf->download($fileName);
    }

    public function downloadpdfAmountMember(Request $request) {
        $startDate = $request->input('entry_date', date('Y-m-d'));
        $endDate = $request->input('end_date', date('Y-m-d'));
    ...
    public function downloadExcelQtyMember(Request $request) {
        $startDate = $request->input('entry_date', date('Y-m-d'));
        $endDate = $request->input('end_date', date('Y-m-d'));
        ...
        if ($startDate == $endDate) {
            $fileName = 'Intermark - Report QTY Member' . date('d F Y', strtotime($startDate)) . '.xlsx';
        } else {
            $fileName = 'Intermark - Report QTY Member' . date('d F Y', strtotime($startDate)) . ' sd ' . date('d F Y', strtotime($endDate)) . '.xlsx';
        }
    
        // return Excel::download(new ReportQtyExport($car_datahours, $motorcycle_datahours, $results, $startDate, $endDate, $judul), $fileName);
        return Excel::download(new ReportQtyMemberExport($results, $car_datahours, $motorcycle_datahours, 
        $car_datahours_in, $motorcycle_datahours_in,
        $startDate, $endDate, $judul), $fileName);
    }

    public function downloadExcelAmountMember(Request $request) {
        $startDate = $request->input('entry_date', date('Y-m-d'));
        $endDate = $request->input('end_date', date('Y-m-d'));
    ...
    $car_datahours_in = DB::select("
        ...
        if ($startDate == $endDate) {
            $fileName = 'Intermark - Report Pendapatan Member' . date('d F Y', strtotime($startDate)) . '.xlsx';
        } else {
            $fileName = 'Intermark - Report Pendapatan Member' . date('d F Y', strtotime($startDate)) . ' sd ' . date('d F Y', strtotime($endDate)) . '.xlsx';
        }
