...
class ReportQtyMemberExport1 implements FromView, WithDrawings, WithTitle, WithStyles
{
    protected $data;
    protected $startDate;
    protected $endDate;
    ...
    {
        return view('exports.reportexcelmemberqty', [
            'data' => $this->data,
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('A2:F' . (count($this->data) + 4))
              ->getBorders()
              ->getAllBorders()
              ->setBorderStyle(Border::BORDER_THIN);
        $sheet->getStyle('A1:F' . (count($this->data) + 4))
              ->getAlignment()
        ...
        $sheet->getStyle('A4:F' . (count($this->data) + 3))->getFont()->setSize(9);
        ...
        foreach ($this->data as $row => $result) {
            ...
            if (date('w', strtotime($result->date)) == 0) {
                $sheet->getStyle("A{$excelRow}:F{$excelRow}")->getFont()->getColor()->setARGB('FFFF0000');
            }
        }
    }
}