...
class ReportQtyPosExport1 implements FromView, WithDrawings, WithTitle, WithStyles
{
    protected $data;
    protected $startDate;
    protected $endDate;
    ...
    {
        return view('exports.reportexcelposqty', [
            'data' => $this->data,
    ...
    {
        return 'Report Pos QTY Amount';
    }

    public function styles(Worksheet $sheet)
    {
        // Apply border to the data range
        $sheet->getStyle('A2:J' . (count($this->data) + 4))
              ->getBorders()
              ->getAllBorders()
              ->setBorderStyle(Border::BORDER_THIN);
        $sheet->getStyle('A1:J' . (count($this->data) + 4))
              ->getAlignment()
        ...
        $sheet->getStyle('A4:J' . (count($this->data) + 3))->getFont()->setSize(9);
        $sheet->getColumnDimension('B')->setWidth(15);

        $sheet->getStyle('B' . (count($this->data) + 5) . ':C' . (count($this->data) + 11))
              ->getBorders()
              ->getAllBorders()
              ->setBorderStyle(Border::BORDER_THIN);
        ...
        foreach ($this->data as $row => $result) {
            ...
            if (date('w', strtotime($result->date)) == 0) {
                $sheet->getStyle("A{$excelRow}:J{$excelRow}")->getFont()->getColor()->setARGB('FFFF0000');
            }
        }
    }
}