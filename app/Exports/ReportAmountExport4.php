...
class ReportAmountExport4 implements FromView, WithDrawings, WithTitle, WithStyles
{
    ...
    {
                $sheet->getStyle('A2:Z' . (count($this->motorcycle_datahours_in) + 3))
              ->getBorders()
              ->getAllBorders()
              ->setBorderStyle(Border::BORDER_THIN);
        $sheet->getStyle('A1:Z' . (count($this->motorcycle_datahours_in) + 3))
              ->getAlignment()
        ...
        $sheet->getStyle('A3:Z' . (count($this->motorcycle_datahours_in) + 3))->getFont()->setSize(9);
